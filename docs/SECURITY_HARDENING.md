...
- S3/CloudWatch regions, buckets, prefixes, log groups, AWS hosts, ECS credential URIs, returned credential shapes, response sizes, redirects, and request timeouts are validated. Regions are selected from a static allowlist; S3 uses fixed regional hosts and encoded path-style bucket addressing so tenant input cannot control outbound DNS.
